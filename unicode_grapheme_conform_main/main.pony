// UAX #29 GraphemeBreakTest.txt conformance runner.
//
// Reads `ucd/auxiliary/GraphemeBreakTest.txt` (path passed as argv[1])
// and verifies our `Graphemes._ranges` iterator places cluster
...
//
// where:
//   ÷ (U+00F7)  = grapheme cluster boundary
//   × (U+00D7)  = NO boundary
...
// Our iterator yields cluster start byte offsets; we translate
...
actor Main
  new create(env: Env) =>
    ...
    let path: String val =
      try env.args(1)?
      else "./ucd/auxiliary/GraphemeBreakTest.txt"
      end
    env.out.print("grapheme conformance: reading " + path)
    ...
    env.out.print(
      "grapheme conformance: " + lines.size().string() + " lines")
    ...
    env.out.print("grapheme conformance: checked " + checked.string()
