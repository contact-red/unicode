...
- `ScriptSet` topical primitive — UAX #39 §5.1 resolved-script-set operations: `resolve` (running intersection of Script_Extensions across constraining codepoints) and `is_single_script` (true iff a string could be a single script, treating Common/Inherited as non-constraining). Detects homograph-shape mixes like Latin A + Cyrillic А
