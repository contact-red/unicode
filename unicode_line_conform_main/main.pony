// UAX #14 LineBreakTest.txt conformance runner.
//
// Reads `ucd/auxiliary/LineBreakTest.txt` (path passed as argv[1])
// and verifies our `Lines._ranges` iterator places cluster
...
// Our iterator yields line start byte offsets; we translate
...
actor Main
  new create(env: Env) =>
    ...
    let path: String val =
      try env.args(1)?
      else "./ucd/auxiliary/LineBreakTest.txt"
      end
    env.out.print("line conformance: reading " + path)
    ...
    env.out.print(
      "line conformance: " + lines.size().string() + " lines")
    ...
    for line in lines.values() do
      ...
      try
        ...
        // Collect actual line-break boundaries from our iterator.
        // LineBreakTest doesn't treat sot as a break opportunity
        // (rule 0.3 in the test annotations); end-of-text is always
        // a mandatory break (LB3).
        let actual_byte = HashSet[USize, HashEq[USize]]
        actual_byte.set(utf8.size())
        match Lines.ranges(utf8)
        | let it: Iterator[(USize, USize)] =>
          var first: Bool = true
          while it.has_next() do
            try
              (let lo: USize, let hi: USize) = it.next()?
              // Skip the very first lo (= 0) — sot isn't an
              // opportunity in LineBreakTest. All other range
              // starts ARE opportunities.
              if not first then actual_byte.set(lo) end
              actual_byte.set(hi)
              first = false
            end
          end
