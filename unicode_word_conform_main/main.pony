// UAX #29 WordBreakTest.txt conformance runner.
//
// Reads `ucd/auxiliary/WordBreakTest.txt` (path passed as argv[1])
// and verifies our `Words._ranges` iterator places cluster
...
// Our iterator yields word start byte offsets; we translate
...
actor Main
  new create(env: Env) =>
    ...
    let path: String val =
      try env.args(1)?
      else "./ucd/auxiliary/WordBreakTest.txt"
      end
    env.out.print("word conformance: reading " + path)
    ...
    env.out.print(
      "word conformance: " + lines.size().string() + " lines")
    ...
    env.out.print("word conformance: checked " + checked.string()
